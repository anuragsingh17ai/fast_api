# if you don't have any idea about secret key just type in terminal `openssl rand -hex 32` this help you to get random hex secreat key
...
async def login_for_acess_token(form_data: Annotated[OAuth2PasswordRequestForm, Depends()],
    ...
    if not user:
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, 
                                detail="could not verify user.")
    token = create_acess_token(user.username, user.role, user.id, timedelta(minutes=20))
